...
export async function getAppVersion() {
  await requireAuth();
  const { getInstalledVersion } = await import('../cron');
  const { getAvailableVersion, getReleaseNotes } = await import('../db/update-check');
  return {
...
export async function triggerUpgrade() {
  await requireAuth();
  const { triggerWorkflowDispatch } = await import('../tools/github');
  const { getAvailableVersion } = await import('../db/update-check');
...
export async function createNewApiKey() {
  const user = await requireAuth();
  try {
    const { createApiKeyRecord } = await import('../db/api-keys');
...
export async function getApiKeys() {
  await requireAuth();
  try {
    const { getApiKey } = await import('../db/api-keys');
    return getApiKey();
  } catch (err) {
...
export async function deleteApiKey() {
  await requireAuth();
  try {
    const mod = await import('../db/api-keys');
    mod.deleteApiKey();
    return { success: true };
  } catch (err) {
...
export async function getSwarmStatus(page = 1) {
  await requireAuth();
  try {
    const { getSwarmStatus: fetchStatus } = await import('../tools/github');
...
export async function getSwarmConfig() {
  await requireAuth();
  const { cronsFile, triggersFile } = await import('../paths');
