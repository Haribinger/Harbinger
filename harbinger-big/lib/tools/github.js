...
async function githubApi(endpoint, options = {}) {
  ...
  if (!res.ok) {
    // Gracefully handle 404 for repo not found / auth issues
    if (res.status === 404) {
      return { ok: false, reason: 'repo_not_found', status: 404 };
    }
...
async function getWorkflowRuns(status, { workflow, page = 1, perPage = 100 } = {}) {
  ...
  if (!GH_OWNER || !GH_REPO) {
    return { workflow_runs: [], total_count: 0, ok: false, reason: 'config_missing' };
  }
  ...
  if (result && result.ok === false && result.reason === 'repo_not_found') {
    return { workflow_runs: [], total_count: 0, ok: false, reason: 'repo_not_found' };
  }
  return result;
}
...
async function getJobStatus(jobId) {
  ...
  // Handle 404/config missing gracefully - return empty state
  if (inProgress.reason === 'repo_not_found' || inProgress.reason === 'config_missing' ||
      queued.reason === 'repo_not_found' || queued.reason === 'config_missing') {
    return { jobs: [], queued: 0, running: 0, ok: false, reason: inProgress.reason || queued.reason };
  }
...
async function getSwarmStatus(page = 1) {
  ...
  // Handle 404/config missing gracefully
  if (data.reason === 'repo_not_found' || data.reason === 'config_missing') {
    return { runs: [], hasMore: false, ok: false, reason: data.reason };
  }
