...
This is an autonomous AI agent powered by [harbinger](https://github.com/stephengpope/harbinger). It uses a **two-layer architecture**:
...
All core logic lives in the `harbinger` npm package. This project is a scaffolded shell — thin Next.js wiring, user-editable configuration, GitHub Actions workflows, and Docker files. When `harbinger` is updated, core logic updates automatically via `npm update`; scaffolded files are managed separately via `npx harbinger init`.
...
│   ├── api/
│   │   ├── [...harbinger]/route.js   # Catch-all API route (re-exports from package)
...
│       ├── Dockerfile                 # Node.js 22, PM2, harbinger package
...
**Event Handler** (this Next.js server): Receives requests (web UI, Telegram, webhooks, cron timers), creates jobs by pushing a `job/<uuid>` branch to GitHub, and manages the web interface. All core logic is in the `harbinger` npm package.
...
All API routes are under `/api/`, handled by the catch-all route at `app/api/[...harbinger]/route.js`.
...
Accessible after login at `APP_URL`. Page shells live in `app/` but all components are imported from the `harbinger` npm package.
...
SQLite via Drizzle ORM at `data/harbinger.sqlite` (override with `DATABASE_PATH` env var). Auto-initialized and auto-migrated on server startup.
...
| `upgrade-event-handler.yml` | Manual `workflow_dispatch` | Creates a PR to upgrade the harbinger package |
...
| `JOB_IMAGE_URL` | Docker image for job agent. GHCR URLs trigger automatic builds via `build-image.yml`. | Default harbinger image |
...
| `DATABASE_PATH` | Override SQLite database location | No (default: `data/harbinger.sqlite`) |
...
| Command | Description |
|---------|-------------|
| `npx harbinger init` | Scaffold or update project — creates missing files, auto-updates managed files, reports drifted user files |
| `npx harbinger setup` | Interactive setup wizard (API keys, GitHub secrets, Telegram bot) |
| `npx harbinger setup-telegram` | Reconfigure Telegram webhook only |
| `npx harbinger reset [file]` | Restore a file to the package default (no args lists available templates) |
| `npx harbinger diff [file]` | Show differences between project files and package templates |
| `npx harbinger reset-auth` | Regenerate AUTH_SECRET (invalidates all sessions) |
| `npx harbinger set-agent-secret <KEY> [VALUE]` | Set a GitHub secret with `AGENT_` prefix and update `.env` |
| `npx harbinger set-agent-llm-secret <KEY> [VALUE]` | Set a GitHub secret with `AGENT_LLM_` prefix |
| `npx harbinger set-var <KEY> [VALUE]` | Set a GitHub repository variable |

### Updating harbinger

```
1. npm update harbinger         — updates the package
2. npx harbinger init           — auto-updates managed files, reports drifted user files
3. npx harbinger diff <file>    — review what changed in a user-editable file
4. npx harbinger reset <file>   — accept the new template, or manually merge
```
...
Use `npx harbinger init --no-managed` to skip auto-updates of managed files.
