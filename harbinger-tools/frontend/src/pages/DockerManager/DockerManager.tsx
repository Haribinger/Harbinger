                      ...
                      <div className="p-4 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-medium">{container.name || container.id?.slice(0, 12) || 'unnamed'}</span>
                            <span className={`text-xs px-2 py-0.5 rounded-full border ${statusColor(container.status)}`}>
                              {container.status}
                            </span>
                          </div>
                          <p className="text-sm text-text-secondary truncate">{container.image || 'no image'}</p>
                        </div>
                ...
                {images.length === 0 ? (
                  ...
                  images.map((img, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="font-medium">{img.name || 'unnamed'}</p>
          ...
          >
            <div className="p-4 border-b border-border">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold">{selectedContainer.name || selectedContainer.id?.slice(0, 12) || 'unnamed'}</h3>
                <button
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full border ${statusColor(selectedContainer.status || 'created')} mt-2 inline-block`}>
            ...
            <div className="p-4 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  {[
                    { label: 'Image', value: selectedContainer.image || 'unknown' },
                    { label: 'ID', value: selectedContainer.id?.slice(0, 12) || 'unknown' },
                    { label: 'Created', value: selectedContainer.createdAt ? new Date(selectedContainer.createdAt).toLocaleString() : 'unknown' },
