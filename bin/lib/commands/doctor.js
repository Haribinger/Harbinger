 ...
 * Checks toolchain, project files, secrets, services, and disk.
...
/**
 * Register a check. `fix` can be a shell command string or an async function.
 */
...
function appendEnv(key, value) {
  ...
  if (re.test(content)) return; // already set
...
addCheck('.env file exists', () => {
  ...
  fs.writeFileSync('.env', '# Harbinger — generated by harbinger doctor --fix\n');
});

addCheck('Required secrets', async () => {
  if (!fs.existsSync('.env')) throw new Error('.env missing');
  const env = fs.readFileSync('.env', 'utf8');
  const required = ['JWT_SECRET', 'DB_PASSWORD', 'REDIS_PASSWORD', 'NEO4J_PASSWORD', 'AUTH_SECRET'];
  ...
  const generators = {
    ...
    NEO4J_PASSWORD: () => randomBytes(16).toString('hex'),
  ...
  for (const [key, gen] of Object.entries(generators)) {
    ...
    if (!env.match(new RegExp(`^${key}=.+`, 'm'))) {
      appendEnv(key, gen());
      console.log(`        ${ICON.ok} Generated ${key}`);
    }
  }
});
...
addCheck('No weak JWT_SECRET', () => {
  ...
  const { randomBytes } = await import('crypto');
  appendEnv('JWT_SECRET', randomBytes(32).toString('hex'));
});
...
addCheck('docker-compose.yml exists', () => {
  if (!fs.existsSync('docker-compose.yml')) throw new Error('docker-compose.yml missing');
});
...
export default async function doctorCommand(args = []) {
    ...
    try {
      ...
      if (fixMode && fix) {
        if (typeof fix === 'function') {
          try {
            console.log(`        ${C.gold('→')} Auto-fixing...`);
            await fix();
  ...
  box('RESULTS', [
    `${C.success(`${passed.length} passed`)}  ${C.danger(`${failed.length} failed`)}  ${C.dim(`${checks.length} total`)}  ${C.gold(`${score}%`)}`,
    '',
    ...(failed.length > 0 ? [
      C.dim('Failed checks:'),
      ...failed.map(f => {
        const fixHint = typeof f.fix === 'function' ? 'auto-fixable' : f.fix || '';
        return `  ${C.danger('·')} ${f.name}${fixHint ? C.dim(` → ${fixHint}`) : ''}`;
      }),
