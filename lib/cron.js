...
function getInstalledVersion() {
  // Try to read from node_modules first (user project with harbinger as dependency)
  const nmPkgPath = path.join(process.cwd(), 'node_modules', 'harbinger', 'package.json');
  if (fs.existsSync(nmPkgPath)) {
    return JSON.parse(fs.readFileSync(nmPkgPath, 'utf8')).version;
  }
  // Fallback: we're IN the harbinger repo itself - read local package.json
  const localPkgPath = path.join(process.cwd(), 'package.json');
  return JSON.parse(fs.readFileSync(localPkgPath, 'utf8')).version;
}
...
async function fetchAndStoreReleaseNotes(target) {
  try {
    const ghRes = await fetch(
      `https://api.github.com/repos/stephengpope/harbinger/releases/tags/v${target}`
    );
    if (!ghRes.ok) return;
  ...
  try {
    ...
    if (isPrerelease(installed)) {
      ...
      const results = await Promise.allSettled([
        fetch('https://registry.npmjs.org/harbinger/latest'),
        fetch('https://registry.npmjs.org/harbinger/beta'),
      ]);
      ...
      const res = await fetch('https://registry.npmjs.org/harbinger/latest');
      if (!res.ok) {
