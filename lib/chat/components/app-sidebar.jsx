...
import { CirclePlusIcon, PanelLeftIcon, MessageIcon, BellIcon, SwarmIcon, ArrowUpCircleIcon, LifeBuoyIcon } from './icons.jsx';
import { getUnreadNotificationCount, getAppVersion } from '../actions';
import { SidebarHistory } from './sidebar-history.jsx';
import { SidebarUserNav } from './sidebar-user-nav.jsx';
import { UpgradeDialog } from './upgrade-dialog.jsx';
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
    ...
    <Sidebar>
      <SidebarHeader>
        ...
        <div className={collapsed ? 'flex justify-center' : 'flex items-center justify-between'}>
          {!collapsed && (
            <span className="px-2 font-semibold text-lg">Harbinger{version && <span className="text-[11px] font-normal text-muted-foreground"> v{version}</span>}</span>
          )}
          <Tooltip>
