...
All event handler logic, API routes, library code, and core functionality lives in the **npm package** (`api/`, `lib/`, `config/`, `bin/`). This is what users import when they `import ... from 'harbinger/...'`.
...
```
/
├── api/                        # Next.js route handlers (exported as harbinger/api)
...
├── templates/                  # Files scaffolded to user projects by `harbinger init`
...
| `bin/cli.js` | CLI entry point (`harbinger init`, `setup`, `reset`, `diff`, `set-agent-secret`, `set-agent-llm-secret`, `set-var`) |
...
|--------|--------|---------|
| `harbinger/api` | `api/index.js` | `GET` and `POST` route handlers — re-exported by the user's catch-all route |
| `harbinger/config` | `config/index.js` | `withThepopebot()` — wraps the user's Next.js config to mark server-only packages as external |
| `harbinger/instrumentation` | `config/instrumentation.js` | `register()` — Next.js instrumentation hook that loads `.env` and starts cron jobs on server start |
| `harbinger/auth` | `lib/auth/index.js` | Auth helpers (`auth()`, `getPageAuthState()`) |
| `harbinger/auth/actions` | `lib/auth/actions.js` | Server action for admin setup (`setupAdmin()`) |
| `harbinger/chat` | `lib/chat/components/index.js` | Chat UI components |
| `harbinger/chat/actions` | `lib/chat/actions.js` | Server actions for chats, notifications, and swarm |
| `harbinger/chat/api` | `lib/chat/api.js` | Dedicated chat streaming route handler (session auth) |
| `harbinger/db` | `lib/db/index.js` | Database access |
| `harbinger/middleware` | `lib/auth/middleware.js` | Auth middleware |
...
| Command | Description |
|---------|-------------|
| `harbinger init` | Scaffold a new project — copies templates, creates `package.json`, runs `npm install` |
| `harbinger setup` | Run interactive setup wizard (API keys, GitHub secrets, Telegram bot) |
| `harbinger setup-telegram` | Reconfigure Telegram webhook only |
| `harbinger reset [file]` | Restore a template file to package default (or list all available templates) |
| `harbinger diff [file]` | Show differences between project files and package templates |
| `harbinger reset-auth` | Regenerate AUTH_SECRET (invalidates all sessions) |
| `harbinger set-agent-secret <KEY> [VALUE]` | Set a GitHub secret with `AGENT_` prefix and update `.env` |
| `harbinger set-agent-llm-secret <KEY> [VALUE]` | Set a GitHub secret with `AGENT_LLM_` prefix |
| `harbinger set-var <KEY> [VALUE]` | Set a GitHub repository variable |

## How User Projects Work

When a user runs `npx harbinger init`, the CLI scaffolds a Next.js project that wires into the package:

1. **`next.config.mjs`** imports `withThepopebot` from `harbinger/config` — marks server-only dependencies as external so they aren't bundled for the client
2. **`instrumentation.js`** re-exports `register` from `harbinger/instrumentation` — Next.js calls this on server start to load `.env`, validate AUTH_SECRET, initialize the database, and start cron jobs
3. **`app/api/[...harbinger]/route.js`** re-exports `GET` and `POST` from `harbinger/api` — catch-all route that handles all `/api/*` requests
4. **`middleware.js`** re-exports auth middleware from `harbinger/middleware` — protects all routes except `/login` and `/api`
5. **`app/api/auth/[...nextauth]/route.js`** re-exports from `harbinger/auth` — handles NextAuth login/session routes
...
harbinger includes a full web interface for managing the agent, accessible after login at `APP_URL`.
...
Page components live in the package (`lib/chat/components/`) and are exported via `harbinger/chat`. Template pages in `templates/app/` are thin wrappers that import these components. The UI uses Tailwind CSS with CSS variables for light/dark theming.
...
harbinger uses SQLite (via Drizzle ORM) for all persistence. The database is stored at `data/harbinger.sqlite` (override with `DATABASE_PATH` env var) and is initialized automatically on first server start.
...
- **rebuild-event-handler.yml** — Triggers on push to `main`. Fast path (build + PM2 reload) or Docker restart path if the harbinger package version changed.
- **upgrade-event-handler.yml** — Manual `workflow_dispatch`. Creates a PR to upgrade the harbinger package in an isolated clone.
...
| `JOB_IMAGE_URL` | Full Docker image path for the job agent (e.g., `ghcr.io/myorg/mybot`). GHCR URLs trigger automatic builds via `build-image.yml`. Non-GHCR URLs (e.g., `docker.io/user/mybot`) are pulled directly. | Not set (uses `stephengpope/harbinger:job-${HARBINGER_VERSION}`) |
| `EVENT_HANDLER_IMAGE_URL` | Docker image path for the event handler | Not set (uses `stephengpope/harbinger-event-handler:latest`) |
...
Users create their agent project with `npx harbinger init` then `npm run setup`. The setup wizard handles API keys, GitHub secrets/variables, and Telegram bot configuration. Users customize their agent by editing:
