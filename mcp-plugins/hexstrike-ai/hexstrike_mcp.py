#!/usr/bin/env python3
"""
Harbinger × HexStrike AI MCP Client v6.0
Enhanced AI Agent Communication Interface — Integrated into Harbinger Platform

Enhanced with AI-Powered Intelligence & Automation
🚀 Bug Bounty | CTF | Red Team | Security Research

RECENT ENHANCEMENTS (v6.0 + Harbinger Integration):
...
✅ Harbinger platform integration — connects to harbinger-hexstrike Docker service
✅ Environment-based server URL (HEXSTRIKE_SERVER env var)
...
Harbinger Integration: Points to http://hexstrike:3001 when running inside Docker stack
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="[🔥 Harbinger × HexStrike MCP] %(asctime)s [%(levelname)s] %(message)s",
    handlers=[
...
for handler in logging.getLogger().handlers:
    handler.setFormatter(ColoredFormatter(
        "[🔥 Harbinger × HexStrike MCP] %(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    ))
...
# When running inside Harbinger Docker stack: http://hexstrike:3001
# When running standalone: http://127.0.0.1:8888
# Override with HEXSTRIKE_SERVER environment variable
DEFAULT_HEXSTRIKE_SERVER = os.environ.get('HEXSTRIKE_SERVER', 'http://127.0.0.1:8888')
...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    def ai_generate_attack_suite(target_url: str, attack_types: str = "xss,sqli,lfi") -> Dict[str, Any]:
        ...
        # Use local variables to avoid Pyright/Pylance type inference issues with heterogeneous dicts
        payload_suites: Dict[str, Any] = {}
        total_payloads: int = 0
        high_risk_payloads: int = 0
        ...
        for attack_type in attack_list:
            ...
            if payload_result.get("success"):
                ...
                payload_suites[attack_type] = payload_data

                # Update counters
                p_count = int(payload_data.get("payload_count", 0))
                t_count = len(payload_data.get("test_cases", []))
                ...
                for payload_info in payload_data.get("payloads", []):
                    if payload_info.get("risk_level") == "HIGH":
                        high_risk_payloads += 1

        logger.info(f"✅ Attack suite generated:")
        logger.info(f"   ├─ Total payloads: {total_payloads}")
        logger.info(f"   ├─ High-risk payloads: {high_risk_payloads}")
        logger.info(f"   └─ Test cases: {test_cases_count}")

        results: Dict[str, Any] = {
            "target_url": target_url,
            "attack_types": attack_list,
            "payload_suites": payload_suites,
            "summary": {
                "total_payloads": total_payloads,
                "high_risk_payloads": high_risk_payloads,
    ...
    def comprehensive_api_audit(base_url: str, schema_url: str = "", jwt_token: str = "", graphql_endpoint: str = "") -> Dict[str, Any]:
        ...
        """
        # Local variables for mutable state to avoid Pyright/Pylance type inference issues
        tests_performed = []
        ...
        if fuzz_result.get("success"):
            tests_performed.append("api_fuzzing")
            phase_results["api_fuzzing"] = fuzz_result
        ...
        if schema_url:
            ...
            if schema_result.get("success"):
                tests_performed.append("schema_analysis")
                phase_results["schema_analysis"] = schema_result

                schema_data = schema_result.get("schema_analysis_results", {})
                total_vulnerabilities += len(schema_data.get("security_issues", []))
        ...
        if jwt_token:
            ...
            if jwt_result.get("success"):
                tests_performed.append("jwt_analysis")
                phase_results["jwt_analysis"] = jwt_result

                jwt_data = jwt_result.get("jwt_analysis_results", {})
                total_vulnerabilities += len(jwt_data.get("vulnerabilities", []))
        ...
        if graphql_endpoint:
            ...
            if graphql_result.get("success"):
                tests_performed.append("graphql_scanning")
                phase_results["graphql_scanning"] = graphql_result

                graphql_data = graphql_result.get("graphql_scan_results", {})
                total_vulnerabilities += len(graphql_data.get("vulnerabilities", []))

        # Assemble the final audit_results
        coverage = "comprehensive" if len(tests_performed) >= 3 else "partial"
        ...
        # Merge phase results into audit_results
        audit_results.update(phase_results)

        logger.info(f"✅ Comprehensive API audit completed:")
        logger.info(f"   ├─ Tests performed: {len(tests_performed)}")
